...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-56 bg-[hsl(0,0%,3.5%)]/80 border-r border-white/[0.06] flex flex-col z-30 backdrop-blur-xl">
      <div className="p-4 border-b border-white/[0.06]">
        <h1
          className="text-lg font-bold tracking-wider cursor-pointer hover:text-white/90 transition-colors bg-gradient-to-r from-white to-white/70 bg-clip-text text-transparent"
          onClick={() => navigate('/')}
        >
          NOCTURN
        </h1>
      ...
      <div className="flex-1 overflow-y-auto py-2">
        {user && profile ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className="w-full px-4 py-3 flex items-center gap-3 hover:bg-white/[0.04] transition-colors text-left">
                <div
                  className="w-8 h-8 rounded-lg bg-white/[0.04] flex items-center justify-center text-xs font-bold overflow-hidden flex-shrink-0 border"
                  style={{ borderColor: profile.rank?.color || '#ffffff22' }}
                >
                  <img
            ...
            <DropdownMenuContent side="bottom" align="start" className="w-48 bg-[hsl(0,0%,4%)]/90 border-white/10 backdrop-blur-xl">
        ...
        <div className="px-2 space-y-0.5">
          <div ref={notifRef} className="relative">
            ...
            {notifOpen && (
              <div className="absolute left-0 top-full mt-1 w-full bg-[hsl(0,0%,4%)]/95 border border-white/10 rounded-xl shadow-xl z-50 max-h-72 overflow-y-auto backdrop-blur-xl">
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <div
                      key={n.id}
                      className={`p-3 border-b border-white/[0.04] text-xs ${!n.read ? 'bg-white/[0.04]' : ''}`}
                    >
                      {n.message}
                    </div>
                  ))
      ...
      <div className="p-3 border-t border-white/[0.06] text-[10px] text-muted-foreground/50 text-center">
        Nocturn Forum
      </div>
    </aside>
  );
