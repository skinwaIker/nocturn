...
export function HomePage() {
  ...
  const pinnedPastes = pastes.filter((p) => p.pinned);
  const regularPastes = pastes.filter((p) => !p.pinned);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-8 text-center">
        ...
        <h1 className="text-3xl font-bold tracking-widest mb-4 bg-gradient-to-r from-white via-white/90 to-white/70 bg-clip-text text-transparent">NOCTURN</h1>
        ...
        <>
          <div className="mb-4">
            <div className="flex items-center gap-2">
              <Pin className="h-3.5 w-3.5 text-white/50" />
              <h2 className="text-sm font-bold text-white/50 uppercase tracking-wider">Pinned</h2>
            </div>
          </div>
          <div className="space-y-2 mb-2">
            {pinnedPastes.map((paste) => (
              <button
                ...
                className="w-full text-left bg-white/[0.03] border border-white/10 rounded-lg p-4 hover:border-white/20 transition-all group backdrop-blur-sm"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <Pin className="h-3 w-3 text-white/40 shrink-0" />
      ...
      {regularPastes.length === 0 && pinnedPastes.length === 0 ? (
        ...
        <div className="text-center py-8 text-muted-foreground">
          <p className="text-sm">No unpinned pastes</p>
        </div>
      ) : (
